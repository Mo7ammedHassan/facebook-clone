...
const checkMember= await groupRepo.getJoinRequestByGroupIdAndUserId(groupId, userId);
...
if(checkMember.status === "pending")
    throw new AppError("You have already sent a request to join this group", 400);

if(checkMember.status === "rejected")
    throw new AppError("Sorry, you are banned from entering this group.", 400);
}
return await groupRepo.requestJoinToGroup(groupId, userId);

}else
    return await groupRepo.addUser(groupId, userId);
};
