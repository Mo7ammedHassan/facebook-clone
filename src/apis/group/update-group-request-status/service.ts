...
const updateGroupRequestStatusService = async (requestId: number,actorId: number,status: GroupStatusRequest) => {
// handel request
...
// handel status
    ...
    if (status === GroupStatusRequest.accepted)
        return await groupRepo.acceptGroupRequest(requestId, groupId, requesterId, actorId);
